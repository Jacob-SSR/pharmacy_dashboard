...
| `GET /api/pharmacy/debug?date=YYYY-MM-DD` | เครื่องมือวินิจฉัยตอนตัวเลขบนจอเป็น 0 — คืนเฉพาะตัวนับกับรหัสแผนก ไม่มีข้อมูลผู้ป่วย |

### ตัวเลขบนจอเป็น 0 ทำยังไง

เปิด `http://<ip>:4600/api/pharmacy/debug` แล้วดูหัวข้อ `วินิจฉัย`

| อาการ | สาเหตุที่พบบ่อย |
| --- | --- |
| "เรียกแล้ว" = 0 | `PHARMACY_DEPCODES` ไม่ตรงกับ `sd_queue_calling_curdep` ที่ใช้จริง — ดูช่อง `เรียกคิวแยกตามแผนก` แล้วเอา depcode นั้นมาตั้ง · หรือโรงพยาบาลไม่ได้ใช้ระบบเรียกคิวที่ห้องยา |
| "รับยาแล้ว" = 0 | เจ้าหน้าที่ไม่ได้ลงเวลารับยา (`service16` = 0) และ HOSxP ไม่ได้เปลี่ยนสถานะ visit เป็นคำที่เข้าข่ายจุดออก — ดูช่อง `สถานะ_visit` ว่าชื่อสถานะจริงคืออะไร แล้วแจ้งมาเพิ่มในลิสต์ |
| ตาราง "ยังไม่มีใบสั่งยา" เยอะผิดปกติ | `PHARMACY_DEPCODES` อาจชี้ไปแผนกที่คนไข้ถูกส่งมาก่อนแพทย์สั่งยา — เทียบกับช่อง `คนไข้ค้างอยู่แผนก_cur_dep` |
