...
flow จริงของห้องยามี 5 ขั้น แต่ HOSxP บันทึกไว้แค่ 3 จุด จอจึงแสดง 3 ขั้น

| ขั้นจริงของห้องยา | มีข้อมูลใน HOSxP | บนจอ |
| --- | --- | --- |
| 1 รับใบสั่งยา (Check + ปริ้นสติกเกอร์ + แบ่งตะกร้า) | ✅ ใบสั่งยาเข้า + `ovst.cur_dep` อยู่ห้องยา | **รับใบสั่งยา** |
| 2 จัดยา (ยาน้อย / ยามาก) | ❌ ไม่มีใครบันทึกว่าเริ่ม/จัดเสร็จ | ยุบเข้าขั้น 1 |
| 3 ตรวจสอบ (Check ก่อนจ่ายยา) | ❌ ไม่มีข้อมูล | ตัดออก |
| 4 จ่ายยา (เรียกชื่อ + จ่ายยา) | ✅ `sd_queue_calling` | **จ่ายยา** |
| ✓ รับยาแล้ว | ✅ `service16` / ออกจากห้องยา | **รับยาแล้ว** |

ขั้น 2-3 เกิดขึ้น "ระหว่างที่คนไข้นั่งรอ" ทั้งคู่ และไม่มีฟิลด์ไหนบอกได้ว่าตอนนี้อยู่ขั้นไหน
แยกได้ก็ต่อเมื่อให้เภสัชกรกดบันทึกเอง แล้วเก็บลงตารางของแอปเอง (ยังไม่ได้ทำ)

> "ยาน้อย / ยามาก" ของขั้น 2 ไม่ได้หายไป — อยู่ที่การ์ดตะกร้าและคอลัมน์ตะกร้า
> ซึ่งเป็นตัวเลขหลักของจอนี้อยู่แล้ว
...
ยกมาจากโปรเจกต์พี่น้อง **`cashier_dashboard`** (จอคิวห้องเก็บเงินของ รพ. เดียวกัน)
ซึ่งใช้งานจริงมาแล้ว — `lib/tts/legacyThaiTTS.ts`, `lib/audio/AudioQueueManager.ts`,
`app/api/tts/route.ts`, `app/useAnnouncer.ts` ยกมาเกือบทั้งดุ้น

```
sd_queue_calling → /api/pharmacy/call → /api/tts → Google TTS → MP3 → <audio> → ลำโพง
```

- **เสียงสร้างที่เซิร์ฟเวอร์** ด้วย Google Translate TTS ตัวเดิมที่จอเก่าใช้ ไม่ใช่
  `speechSynthesis` ของเบราว์เซอร์ — เพราะทีวีแต่ละยี่ห้อมีเสียงไทยคนละตัว
  ย้ายมาสร้างที่เซิร์ฟเวอร์แล้วทุกจอได้เสียงเดียวกัน และเป็นเสียงที่คนไข้คุ้นอยู่แล้ว
- **อ่านอย่างเดียว ไม่เขียน HOSxP** ต่างจากจอ PHP เดิมที่ `UPDATE status='N'`
  ที่นี่ใช้ `key = แฮช(vn + เวลาที่ถูกเรียก)` แล้วให้จอจำ key ที่ประกาศแล้วไว้เอง
  → **รันคู่กับจอเดิมได้ ไม่แย่งธงกัน**
- **ไม่ประกาศรอบแรก** เปิดจอ/รีเฟรชหน้า ไม่ตะโกนชื่อคนที่ยืนอยู่หน้าเคาน์เตอร์แล้ว
- **กดเรียกคนเดิมซ้ำได้** เวลาเรียกเปลี่ยน → key เปลี่ยน → ประกาศใหม่
- **เสียงไม่ทับกัน** ต่อคิวเล่นทีละอัน (`AudioQueueManager`)

เปิดใช้ 2 ขั้น

1. `PHARMACY_VOICE=1` ใน `.env.production` แล้ว `docker compose up -d --force-recreate app`
2. เปิดจอตัวประกาศด้วย `?voice=1` เช่น `http://<ip>:4600/tv?voice=1`
   แล้ว **กดอะไรก็ได้บนจอ** (คลิก แตะ หรือปุ่มไหนก็ได้บนรีโมต) เพื่อปลดล็อกเสียง
   — เบราว์เซอร์บล็อกเสียงอัตโนมัติจนมีการกดสักครั้ง

**เปิด `?voice=1` จอเดียวในห้อง** ไม่งั้นเสียงซ้อนกัน

### เสียงไม่ออก เช็คตรงนี้

```
http://<ip>:4600/api/tts?text=ทดสอบ&debug=1
```

- `ok: true` = เซิร์ฟเวอร์สร้างเสียงได้ ปัญหาอยู่ที่จอ (ยังไม่ได้กดปลดล็อก / ลำโพงทีวี)
- `ok: false` = เครื่องที่รัน docker ออกเน็ตไปหา `translate.google.com` ไม่ได้
  หรือถูก Google ตอบ 403 — ดู `docker compose logs -f app` หา `[tts]`
...
| `GET /api/pharmacy/call` | คนที่ถูกกดเรียกล่าสุดที่จุดจ่ายยา · อ่านอย่างเดียว |
| `GET /api/tts?text=...` | ไฟล์ MP3 ของประโยคนั้น (`&debug=1` ดูสาเหตุเวลาเสียงไม่ออก) |
