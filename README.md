...
ทำตาม `getMedicineQ.php` เดิม — อ่านแถวที่ `sd_queue_calling_status='Y'`,
ประกาศ "ขอเชิญ คุณ&lt;ชื่อ&gt; &lt;นามสกุล&gt; ที่จุด &lt;แผนก&gt; ค่ะ" ผ่าน Google TTS,
แล้ว `UPDATE status='N'` กันประกาศซ้ำ

> ⚠️ **นี่เป็นจุดเดียวในแอปที่เขียนลง HOSxP** ปิดไว้เป็นค่าเริ่มต้น
> เปิดด้วย `PHARMACY_VOICE=1`
>
> **ถ้ายังเปิดจอ PHP เดิมอยู่ด้วย สองระบบจะแย่งกันประกาศ** — ใครพลิกธงก่อนได้ประกาศ
> อีกฝ่ายจะเงียบ เลือกใช้อย่างใดอย่างหนึ่ง

เปิดใช้ 2 ขั้น

1. ตั้ง `PHARMACY_VOICE=1` ใน `.env.production` แล้ว `docker compose up -d --force-recreate app`
2. เปิดจอที่จะเป็นตัวประกาศด้วย `?voice=1` เช่น `http://<ip>:4600/tv?voice=1`
   แล้ว **กดปุ่ม "🔊 เปิดเสียงเรียกชื่อ" หนึ่งครั้ง** (browser บล็อกเสียงอัตโนมัติจนผู้ใช้กดก่อน)

**เปิด `?voice=1` ที่จอเดียวเท่านั้น** — เปิดหลายจอจะแย่งกันประกาศเหมือนกรณีข้างบน

ต่างจากสคริปต์เดิม 3 จุด (รัดกุมขึ้น ไม่ได้เปลี่ยนพฤติกรรมที่ต้องการ)

| | ของเดิม | ที่นี่ |
| --- | --- | --- |
| จำนวนคนต่อครั้ง | ต่อชื่อหลายคนเป็นก้อนเดียว | ทีละคน — Google TTS ตัดข้อความยาวเกิน ~200 ตัวอักษร ของเดิมเสี่ยงชื่อขาดกลางประโยค |
| ขอบเขต `UPDATE` | `hn` + เลขคิว | `vn` + วันที่ + depcode — ของเดิมอาจพลิกธงของ visit วันอื่น/แผนกอื่นที่ hn กับเลขคิวชนกัน |
| Google TTS ล่ม | ไม่มีเสียง | ใช้เสียงของ browser (`speechSynthesis`) แทน |
...
| `POST /api/pharmacy/announce` | เอาคิวที่รอประกาศคนถัดไป + mp3 · **เขียน** `sd_queue_calling_status='N'` · ปิดไว้ถ้าไม่ตั้ง `PHARMACY_VOICE=1` |
